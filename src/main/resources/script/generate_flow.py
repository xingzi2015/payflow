...
def generate_flowchart():
    for flow in parsed_data['payflow']['flows']:
        ...
        for node in flow['nodes']:
            flowchart.node(node['id'], label=node['id'] + "(" + extract_action_name(node['create-exp']) + ")",
                           shape='box')
            if node.get('start'):
            ...
            if node.get('conditions'):
                for condition in node['conditions']:
                    if (condition is None):
                        ...
                        create_exp_list = ["Not (" + extract_action_name(item['create-exp']) + ")" if item.get(
                            'is-negated') else extract_action_name(item['create-exp']) for item in
                                           condition.get('node-whens', [])]
